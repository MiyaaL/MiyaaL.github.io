...
let catalogCache: { expiresAt: number; documents: CatalogDocument[] } | null =
  null;
...
function corsHeaders(origin: string): HeadersInit {
  return {
    ...
    "Access-Control-Allow-Headers":
      "apikey, authorization, if-range, range, x-client-info",
...
function isProxySource(entry: CatalogDocument): boolean {
  return entry.source === "external" || entry.source === "release";
}

async function proxiedDocumentUrl(id: string): Promise<URL> {
  let documents = await loadCatalog();
  let document = documents.find((entry) =>
    entry.id === id && isProxySource(entry)
  );
  if (!document) {
    documents = await loadCatalog(true);
    document = documents.find((entry) =>
      entry.id === id && isProxySource(entry)
    );
  }
  if (!document?.path) {
  ...
  const url = validatedPdfUrl(document.path);
  if (document.source === "release") {
    const prefix =
      "/MiyaaL/MiyaaL.github.io/releases/download/library-assets-v1/";
    if (
      url.hostname !== "github.com" || !url.pathname.startsWith(prefix) ||
      url.search
    ) {
      throw new Error("external_url_not_allowed");
    }
