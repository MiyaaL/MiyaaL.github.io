...
- 现有周期的编辑默认固定截止日；首次保存或修改排期参数时，使用 `replanRemaining` 从指定日期按周模板重建剩余排期，清除剩余课次的改期和旧整体顺延，保留此前排期和所有日志。节假日选项默认照常训练，手动休息日仍优先。已重排后仅改重量或训练时间时保留手动改期。重排前已完成的主项减量 / 恢复课会作为新波次起点，避免稀疏历史导致重复减量。重排后的结束日期即固定截止日，只能在日期范围内逐次改期；要改变截止日须明确编辑设置。若任何日志已在新截止日之后，会拒绝重排。
- `priorities` 决定哪些目标参与周期风险提示，默认是卧推和深蹲；各主项仍独立判断测试就绪，它不改变周期边界。设置中可选择其他优先项。
- 预计达标时间默认暂按每周 0.5% 的能力变化估计；有至少跨两周的近期有效记录时使用观测速度，并限制在 0.25%–1%/周。该预测只用于风险提示，不改周期边界或直接决定训练重量。
- 当前估算 1RM 只由有效记录更新；普通、恢复和减量训练均以当前能力计算负荷，不按日历向目标线性抬高。实际 RPE 过高或失败仍会下调下一次负荷；跳过的课不推进训练波次，同一主项间隔超过 10 天先恢复训练。
...
- 单次训练使用完成的 1–5 次、明确填写的实际 RPE 7–10 主项组估算，以较少次数组为优先，同日仅一条观察。辅助引体允许负的额外重量，只要体重加器械负荷仍为正。减量周和恢复训练不参与估算；测试前减量准备中的校准组仅在符合相同的次数与实际 RPE 条件时参与，以便临近测试仍能更新能力和就绪判断；缺失 RPE 不补成计划值。schema v3 迁移前没有来源标记的 RPE 不视为实测，迁移时以已保存的当前能力作为评估锚点。最近三次有效观察取中位数，只有一次观察时与固定基准平滑。
- 能力从原始记录重新计算，因此重复保存、改备注、补录旧训练不会重复累计。手动修改当前能力会创建带日期的新评估基准，保留周期初始基线和原始日志。
- 每个主项按实际排期中的最后一课倒推 7 天减量准备，不按原程序周次提前两周减量；长时间中断后的首课仍优先恢复，不直接测试。最后一课安排周期目标，远期列出 90% / 95% / 100% 三档计划尝试。进入测试前 21 天，须有近期至少两个训练日的有效记录支持；未就绪或最近失败则改按当前能力做 2×3 评估，目标不变。每一档都必须在上一把稳定且仍有余力时继续。
