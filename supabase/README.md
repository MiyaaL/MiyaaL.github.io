...
- 预计达标时间默认暂按每周 0.5% 的能力变化估计；有至少跨两周的近期有效记录时使用观测速度，并限制在 0.25%–1%/周。这只用于提示目标可能无法在本周期内完成，不会生成超出设定结束日期的训练，也不用于抬高训练重量。
...
- 目标测试需要近期至少两个不同训练日支持，最近失败会撤销准入。远期日程标为待确认评估，测试前须重新确认；目标尝试也保留现场停止条件。
- 减量同步减少辅助组数和目标 RPE，测试日取消辅助训练。辅助动作仅按该节主动作类型归类，改期与优先目标不会跨类型注入动作；容量推使用暂停卧推、绳索下压和侧平举。
