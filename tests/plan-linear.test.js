...
(function progressiveWeightsAndOneValleyAreExplicit() {
  ...
  assert.deepStrictEqual(bench.map((session) => session.workout.workSets[0].loadKg), [67.5, 72.5, 80]);
  assert.deepStrictEqual(bench.map((session) => [session.workout.workSets[0].sets, session.workout.workSets[0].reps]), [[4, 5], [4, 4], [4, 3]]);
  for (const dates of [["2026-09-29", "2026-10-06", "2026-10-13"], ["2026-09-30", "2026-10-07", "2026-10-14"], ["2026-10-02", "2026-10-09", "2026-10-16"]]) {
    const loads = dates.map((date) => sessionAt(plan, date).workout.workSets[0].loadKg);
  ...
  assert.strictEqual(sessionAt(plan, "2026-10-26").workout.workSets[0].loadKg, 80, "test week does not add another reduction");
...
(function sparseLogsAndActualRpeNeverRestartOrAutomaticallyUnloadThePlan() {
  ...
  const completed = sessionAt(initial, "2026-09-14");
  ...
  assert.deepStrictEqual(sessionAt(plan, "2026-09-14").workout, completed.workout, "historical prescription is frozen");
...
(function theDefaultPlanAlsoUsesTheSingleLinearRule() {
  ...
  const plan = core.generate(state, [holidays], { asOfDate });
  assert.strictEqual(sessionAt(plan, "2026-09-28").workout.workSets[0].loadKg, 67.5);
